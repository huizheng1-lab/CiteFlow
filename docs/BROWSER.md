...
### Preserve a bibliography with unmatched entries

Version 0.6.5 compares the original bibliography with embedded Mendeley source records before regenerating it. If any entry is unmatched or ambiguous, the complete original bibliography and citation displays remain intact. The review panel reports original entries separately from linked sources and lists unmatched text. Download/reopen and prose edits retain all entries and original numbering. Citation edits, citation deletion/reordering, style changes, bibliography replacement, and collaborator conversion are paused so incomplete metadata cannot silently change references. Correct missing links in the original document with Mendeley and reopen it to enable reformatting. No missing source metadata is invented. If a copy made by an earlier version has already lost entries, reopen the original Word file; the reduced copy cannot supply missing bibliography text.
