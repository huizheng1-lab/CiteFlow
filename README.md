...
In **Local library**, click **Include in resources** to add or reuse a source without inserting a citation. Then select a manuscript insertion point and use **Cite here** in **Sources**. Exact duplicates are automatically reused. Nearly identical references (including matching identifiers with differing metadata) are shown for review: check the new records you want and click **Keep selected**. Existing records remain unchanged; use **Merge duplicates** to replace an existing document source. You can select multiple records to retain distinct versions. Cancel leaves the library/document unchanged.
