...
function makeWorker() {
  worker = new Worker(new URL('./document-worker.js?v=0.6.9', import.meta.url), { type: 'module' });
...
$('#handoff-target').onchange = () => {
  $('#handoff-details').textContent =
    $('#handoff-target').value === 'endnote'
      ...
      : 'Mendeley Cite: exports modern citation controls with embedded references and the existing bibliography in place. Open the copy with Mendeley Cite and verify editing and updating the existing list. Intended for the modern add-in, not the legacy Desktop plugin.';
};
